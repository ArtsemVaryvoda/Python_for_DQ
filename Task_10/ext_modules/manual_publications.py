...
class News(RecordGeneratorManual):
    def __init__(self):
        ...
        self.db_record = [self.text, self.city_name, self.publish_date]
...
class PrivateAd(RecordGeneratorManual):
    def __init__(self):
        ...
        if dates_diff.days < 0:
            ...
            sys.exit()
        self.db_record = [self.text, str(self.ad_valid_until.strftime('%Y/%m/%d')), dates_diff]
        DB(self.db_record, self.__class__.__name__)
        self.record = self.create_header() + '\n' + self.text + '\n' \
...
class Congratulations(RecordGeneratorManual):
    def __init__(self):
        ...
        self.db_record = [self.birthday_text, self.birthday_year, datetime.now().year - int(self.birthday_year)]
        DB(self.db_record, self.__class__.__name__)
        self.record = self.create_header() + "\n" + self.text + "\n" \
